...
void create_backups(int index) {
    ...
    for(int i=0;i<storage_servers[index].path_count;i++)
    {
        strcpy(storage_servers[0].duplicate[index].filepaths[i], storage_servers[index].accessible_paths[i]);
        strcpy(storage_servers[1].duplicate[index].filepaths[i], storage_servers[index].accessible_paths[i]);
    }
    storage_servers[0].duplicate[index].path_count=storage_servers[index].path_count;
    storage_servers[1].duplicate[index].path_count=storage_servers[index].path_count;
    for (int i = 0; i < storage_servers[index].path_count; i++) 
    {
        char new_path[MAX_PATH_LENGTH] = ""; // Path up to the current file or directory
        char temp_path[MAX_PATH_LENGTH] = ""; // Name of the file or directory
        char* full_path = storage_servers[index].duplicate[index].filepaths[i];
        ...
        while(path!=NULL)
        {
            ...
            if(strstr(path, ".")==0)
                is_folder=0;
            create_file_or_folder_on_ns_1(new_path, path, is_folder);
            strcat(new_path, "/");
...
int acquire_read_lock(const char *filepath) {
    ...
    // First, try to find existing lock for this file
    for (int i = 0; i < file_lock_count; ++i) {
        if (strcmp(file_locks[i].filepath, filepath) == 0) {
            ...
            // Wait until no write lock is active
    ...
    // File lock doesn't exist, create a new one
    if (file_lock_count >= MAX_FILES) {
        pthread_mutex_unlock(&file_locks_mutex);
        return -1; // No space for new locks
    }
    
    // Initialize new file lock
    strncpy(file_locks[file_lock_count].filepath, filepath, MAX_PATH_LENGTH - 1);
    file_locks[file_lock_count].filepath[MAX_PATH_LENGTH - 1] = '\0';
    file_locks[file_lock_count].write_locked = 0;
    file_locks[file_lock_count].reader_count = 1; // This reader
    
    pthread_mutex_init(&file_locks[file_lock_count].lock, NULL);
    pthread_cond_init(&file_locks[file_lock_count].read_cond, NULL);
    pthread_cond_init(&file_locks[file_lock_count].write_cond, NULL);
    ...
    for (int i = 0; i < file_lock_count; ++i) {
        if (strcmp(file_locks[i].filepath, filepath) == 0) {
            ...
            if (file_locks[i].reader_count == 0) {
                // No more readers, signal waiting writers
...
int delete_file_or_folder(const char *filePath) {
    ...
    if(is_up[found]==1)
    {
        ...
        int result1= forward_delete_request_to_ss_backup(&(storage_servers[0]), filePath);
        int result2= forward_delete_request_to_ss_backup(&(storage_servers[1]), filePath);
        if (result == 0) 
        {
                ...
                for (int j = 0; j < storage_servers[i].path_count; j++) {
                    if (strcmp(storage_servers[i].accessible_paths[j], filePath) == 0) {
                        ...
                        for (int k = j; k < storage_servers[i].path_count - 1; k++) {
                            ...
                            strcpy(storage_servers[0].duplicate[i].filepaths[k], storage_servers[0].duplicate[i].filepaths[k+1]);
                            strcpy(storage_servers[1].duplicate[i].filepaths[k], storage_servers[1].duplicate[i].filepaths[k+1]);
                        }
                        storage_servers[i].path_count--;  // Decrease the path count
                        storage_servers[0].duplicate[i].path_count--;
                        storage_servers[1].duplicate[i].path_count--;
...
int forward_create_request_to_ss1(StorageServer server, const char *filePath, const char *name, int is_folder) {
    int sock;
    struct sockaddr_in server_addr;
    char message[256];
    char response[10];
...
int is_path_accessible1(const char* filePath) {
    // Iterate over each storage server
    for (int i = 0; i < server_count; i++) {
        StorageServer server = storage_servers[i];
        // Iterate over each accessible path in the current server
...
int create_file_or_folder_on_ns_1(const char *filePath, const char *name, int is_folder) {
    char log_data[4096];
    char new_path[4096];
    // Check if the filePath is accessible
    if (!is_path_accessible1(filePath)) {
    ...
    int result = forward_create_request_to_ss1(storage_servers[0], filePath, name, is_folder);
    int result1 = forward_create_request_to_ss1(storage_servers[1], filePath, name, is_folder);
    if (result != 0) {
    ...
    strcpy(storage_servers[0].duplicate[selectedIndex].filepaths[storage_servers[0].duplicate[selectedIndex].path_count], new_path);
    strcpy(storage_servers[1].duplicate[selectedIndex].filepaths[storage_servers[1].duplicate[selectedIndex].path_count], new_path);
    ...
    //while(1)
    {
        ...
        if (strcmp(operation, "READ") == 0 || strcmp(operation, "STREAM") == 0 || 
            strcmp(operation, "WRITE") == 0 || strcmp(operation, "INFO") == 0) {
                ...
                int lock_acquired = 0;
                if (strcmp(operation, "READ") == 0 || strcmp(operation, "STREAM") == 0 || strcmp(operation, "INFO") == 0) {
                    // For read operations, acquire read lock (allows multiple concurrent readers)
                    if (acquire_read_lock(filePath) == 0) {
                        lock_acquired = 1;
                        snprintf(buffer_log, sizeof(buffer_log), "Read lock acquired for file: %s", filePath);
                        logging_data(buffer_log);
                    } else {
                        snprintf(buffer_log, sizeof(buffer_log), "Failed to acquire read lock for file: %s", filePath);
                        logging_data(buffer_log);
                        const char *error_response = "FILE_LOCKED";
                        send(client_socket, error_response, strlen(error_response), 0);
                        close(client_socket);
                        return;
                    }
                } else if (strcmp(operation, "WRITE") == 0) {
                    // For write operations, acquire exclusive write lock
                    if (acquire_write_lock(filePath) == 0) {
                        lock_acquired = 1;
                        snprintf(buffer_log, sizeof(buffer_log), "Write lock acquired for file: %s", filePath);
                        logging_data(buffer_log);
                    } else {
                        snprintf(buffer_log, sizeof(buffer_log), "Failed to acquire write lock for file: %s", filePath);
                        logging_data(buffer_log);
                        const char *error_response = "FILE_LOCKED_FOR_WRITING";
                        send(client_socket, error_response, strlen(error_response), 0);
                        close(client_socket);
                        return;
                    }
                ...
                if(strcmp(operation, "READ") == 0 && is_up[index_server]==0)
                {
                    ...
                    if(is_up[0]==1)
                        send(client_socket, &storage_servers[0], sizeof(StorageServer), 0);
                    else if(is_up[1]==1)
                        send(client_socket, &storage_servers[1], sizeof(StorageServer), 0);
                    else if(is_up[0]==0 && is_up[1]==0)
                    {
                        send(client_socket, &storage_servers[1], sizeof(StorageServer), 0);
                        errorCode=200;
                    }
            ...
            strcmp(operation, "WRITE") == 0 || strcmp(operation, "INFO") == 0))
                ...
                // Release the appropriate lock after operation completion
                if (lock_acquired) {
                    if (strcmp(operation, "READ") == 0 || strcmp(operation, "STREAM") == 0 || strcmp(operation, "INFO") == 0) {
                        if (release_read_lock(filePath) == 0) {
                            snprintf(buffer_log, sizeof(buffer_log), "Read lock released for file: %s", filePath);
                            logging_data(buffer_log);
                        }
                    } else if (strcmp(operation, "WRITE") == 0) {
                        if (release_write_lock(filePath) == 0) {
                            snprintf(buffer_log, sizeof(buffer_log), "Write lock released for file: %s", filePath);
                            logging_data(buffer_log);
                        }
                    }
                }
            ...
            int result1 = create_file_or_folder_on_ns_1(filePath, name, is_folder);
...
int main(int argc, char const *argv[]) {
    ...
    // const char *path = "~/Desktop/course-project-team_12-main(2)/course-project-team_12-main/backup1";  // Specify the path to the directory

    // if (remove_file_or_directory(path) != 0) {
    //     fprintf(stderr, "Failed to remove directory and its contents\n");
    //     return 1;
    // }
    // path = "~/Desktop/course-project-team_12-main(2)/course-project-team_12-main/backup2";
    // if (remove_file_or_directory(path) != 0) {
    //     fprintf(stderr, "Failed to remove directory and its contents\n");
    //     return 1;
    // }
